...
constexpr char SIGNAL[] = "A";
constexpr char SENSOR_TOPIC[] = "traffic/signalA/sensor";
constexpr char STATUS_TOPIC[] = "traffic/signalA/status";
...
void onMqttMessage(char* topic, byte* data, unsigned int length) {
  ...
  Serial.printf("MQTT priority command: Signal A GREEN for %lu ms\n", controller.activeGreenMs());
}
void writeOutputs() {
  const LocalLampState lamps = controller.lampsForA();
...
void publishStatus() {
  char payload[224];
  snprintf(payload, sizeof(payload), "{\"signal\":\"A\",\"phase\":%u,\"green_time\":%lu,\"sensor_1\":%s,\"sensor_2\":%s,\"fallback\":%s}",
...
void connectMqtt(uint32_t nowMs) {
  ...
  char clientId[32]; snprintf(clientId, sizeof(clientId), "traffic-signal-A-%06X", static_cast<unsigned>(ESP.getEfuseMac()));
...
void serviceSerialDebug(uint32_t nowMs) {
  while (Serial.available() > 0) {
    ...
    if (command == 'f' || command == 'F') { controller.enterFailsafe(); Serial.println("Signal A FAILSAFE ALL-RED latched"); }
    if ((command == 'r' || command == 'R') && controller.isFailsafe()) { controller.begin(nowMs); Serial.println("Signal A failsafe reset"); }
  }
}
void setup() {
  Serial.begin(115200);
  ...
  Serial.printf("Signal A started; MQTT control topic: %s\n", CONTROL_TOPIC);
}
void loop() {
